    ...
    <div className="min-h-screen flex flex-col font-outfit text-stone-800 antialiased overflow-x-hidden selection:bg-rose-500 selection:text-white">
      ...
      <footer className="bg-stone-900 text-stone-300 mt-16 border-t border-stone-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          {/* STUNNING PROMOTIONAL FOOTER MEGA BANNER */}
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-stone-950 via-rose-950/40 to-stone-950 border border-rose-500/20 p-8 sm:p-12 mb-12 shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
              ...
              <div className="max-w-xl">
                <div className="flex items-center gap-2 mb-3">
                  <span className="bg-rose-500/10 border border-rose-500/30 text-rose-400 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full flex items-center gap-1.5">
                    <Sparkles className="w-3 h-3 text-rose-400 fill-rose-400" />
                    <span>VIP Member Blueprint</span>
                  </span>
                  <span className="text-xs text-stone-400 font-medium">• Limited Confectionery Drop slots</span>
                </div>
                
                <h3 className="font-playfair text-2xl sm:text-3xl font-bold text-white leading-tight mb-3">
                  Join the Downtown VIP Connoisseur Guild
                </h3>
                
                <p className="text-xs sm:text-sm text-stone-300 leading-relaxed">
                  Gain priority pre-order access to premium designer tiers, unlisted single-origin truffles, custom milestone piping matrix integrations, and an instant <strong className="text-rose-400 font-bold">15% Welcome Privilege Voucher</strong>.
                </p>
                
                {/* Micro guarantees strip */}
                <div className="flex flex-wrap items-center gap-4 mt-4 pt-4 border-t border-stone-800/80 text-[11px] text-stone-400">
                  <div className="flex items-center gap-1">
                    <Check className="w-3.5 h-3.5 text-rose-500" />
                    <span>Zero spam architecture</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <Check className="w-3.5 h-3.5 text-rose-500" />
                  ...
                  <div className="flex items-center gap-1">
                    <Check className="w-3.5 h-3.5 text-rose-500" />
                    <span>Direct WhatsApp pipeline</span>
                  </div>
                </div>
              </div>

              {/* Form / Live Response State Block */}
              <div className="w-full lg:w-auto bg-stone-900/90 border border-stone-800/80 p-6 rounded-2xl backdrop-blur-md shrink-0 lg:min-w-[380px]">
                <p className="text-xs font-bold text-white uppercase tracking-wider mb-3 flex items-center gap-1.5">
                  <span>📥 Instantly Secure Invite</span>
                </p>
                ...
                {bannerSubscribed ? (
                  <motion.div 
                    initial={{ opacity: 0, scale: 0.95 }}
                    animate={{ opacity: 1, scale: 1 }}
                    className="bg-emerald-500/10 border border-emerald-500/30 p-4 rounded-xl flex items-start gap-3 text-emerald-400 text-left"
                  >
                    <div className="w-6 h-6 rounded-full bg-emerald-500/20 flex items-center justify-center shrink-0 mt-0.5">
                      <Check className="w-3.5 h-3.5 text-emerald-400" />
                    </div>
                    <div>
                      <p className="text-xs font-bold text-white">Privileges Dispatched!</p>
                      <p className="text-[11px] text-emerald-300/80 mt-1 leading-tight">
                        Pipeline active for <span className="font-mono text-white underline">{bannerEmail}</span>. Use verified token <span className="font-mono font-bold text-white bg-emerald-500/20 px-1 py-0.5 rounded">DTOWN15VIP</span> at checkout simulator.
                      </p>
                    </div>
                  </motion.div>
                ) : (
                  <div className="space-y-3">
                    <div>
                      <input 
                        type="email"
                        placeholder="Enter premium contact email..."
                        value={bannerEmail}
                        onChange={(e) => setBannerEmail(e.target.value)}
                        className="w-full bg-stone-950 border border-stone-700 focus:border-rose-500 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder:text-stone-500 focus:outline-none transition-all font-medium"
                      />
                    </div>
                    <button 
                      onClick={() => {
                        if (bannerEmail.trim() && bannerEmail.includes('@')) {
                          setBannerSubscribed(true);
                          triggerToast('Welcome Privilege activated!');
                        } else {
                          triggerToast('Please provide a verified contact link');
                        }
                      }}
                      className="w-full bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-500 hover:to-pink-500 text-white font-bold text-xs py-3 rounded-xl shadow-lg shadow-rose-600/20 transition-all transform active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <span>Claim Welcome Privilege</span>
                      <ChevronRight className="w-4 h-4" />
                    </button>
                    <p className="text-[10px] text-stone-500 text-center leading-none">
                      🔒 Your link is cryptographically protected and never resold.
                    </p>
                  </div>
                )}
              </div>
